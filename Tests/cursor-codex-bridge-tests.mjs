...
test("HTTP SDK compaction with an expired previous_response_id still completes the stream", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "cursor-sdk-compaction-expired-id-"));
  const workspace = path.join(root, "workspace");
  const fixture = fakeCursorSDK();
  ...
  try {
    ...
    const compactedRequest = baseRequest({
      previous_response_id: "resp_expired_compaction",
      instructions: undefined,
      input: [{
        type: "compaction",
        id: "cmp_expired",
        ...
        role: "user",
        type: "message",
        content: "continue from expired previous_response_id",
      }],
      tools: [],
      stream: true,
    });
    ...
    assert.match(compactedResponse.headers.get("content-type") ?? "", /text\/event-stream/);
    assert.match(body, /event: response\.completed/);
    assert.doesNotMatch(body, /invalid_previous_response/);
    assert.match(String(fixture.observed.sends.at(-1).message), /opaque-compaction-state/);
    assert.match(metrics.at(-1).continuation_source, /previous_response_id_replay/);
    assert.equal(metrics.at(-1).sdk_compaction_boundary, true);

    const staleUserOnly = await fetch(url, {
      method: "POST",
      headers,
      body: JSON.stringify(baseRequest({
        previous_response_id: "resp_still_unknown",
        input: [{ role: "user", type: "message", content: "no history" }],
        tools: [],
        stream: false,
      })),
    });
    assert.equal(staleUserOnly.status, 409, await staleUserOnly.text());
  } finally {
    await stopBridge(server);
    await rm(root, { recursive: true, force: true });
  }
