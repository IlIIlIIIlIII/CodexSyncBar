...
final class CodexCursorModelCatalogService {
    ...
    /// Rewrites the managed catalog only when generated bytes differ.
    /// Returns whether the on-disk file changed.
    func refreshIfChanged(cursorCatalog: CursorModelCatalog) async throws -> Bool {
        let candidate = try await generatedCatalog(cursorCatalog: cursorCatalog)
        if pathEntryExists(catalogURL) {
            let size = try requireOwnedRegularFile(catalogURL)
            guard size >= 0,
            ...
            let existing = try Data(contentsOf: catalogURL)
            guard existing.count <= CodexCursorModelCatalogBuilder.maximumGeneratedCatalogBytes else {
                throw AppError.processFailed("기존 Cursor 모델 카탈로그가 너무 큽니다.")
            }
