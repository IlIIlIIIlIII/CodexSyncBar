...
- [ ] **Step 3: Implement the minimal parser and fixture**
...
`ReadmeDemoFixture.standard`는 기준 시각 `2026-01-15T12:00:00Z`, 위 Global Constraints의 계정·호스트, 별칭 `메인`/`서브`, 장치 표시명 `작업 서버`/`빌드 서버`, 주간 잔여량 `68%`/`42%`, 고정 토큰 합계 `12,345,678`을 사용한다.
...
- Produces: `AppModel.applyReadmeDemoFixture(_:)`
...
Expected: `applyReadmeDemoFixture`가 없어 컴파일 실패한다.

- [ ] **Step 3: Implement the minimal demo model path**
...
- [ ] **Step 4: Implement app-owned view capture**
...
- [ ] **Step 4: Privacy and visual verification**

Run: `rg -n 'younsunggu|yyssgg|100\\.(115|90|70)|/Users/sunggu|sunggu@' README.md docs/images Sources/CodexSyncBar/ReadmeDemo.swift Scripts/capture-readme-ui.sh`

Expected: 결과 없음. 두 PNG를 직접 열어 텍스트 잘림, 스크롤, 빈 공간과 비정상 배율이 없는지 확인한다.

- [ ] **Step 5: Final verification and publish**
